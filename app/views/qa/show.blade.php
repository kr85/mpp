...
<div class="qwrap">
		...
		@if($question->tags!=null)
			<ul class="qtagul">
				@foreach($question->tags as $tag)
					<li>{{HTML::linkRoute('tagged',$tag->tag,$tag->tag_name)}}</li>
				@endforeach
			</ul>
		@endif
			...
			<div class="qwrap">
				<ul class="fastbar">
					@if(Sentry::getUser()->hasAccess('admin'))
						<li class="close">{{HTML::linkRoute('question.delete','delete',$question->id)}}</li>
					@endif
	...
	@if(Sentry::check())
		<div class="rrepol" id="replyarea" style="margin-bottom:10px">
			{{ Form::open(array('route' => array('answer.post', $question->id, Str::slug($question->title)))) }}
                <p class="minihead">
                    Provide your Answer:
                </p>
                <p>
